      ...
      return (
        <div className="bg-white p-3 border border-gray-300 rounded shadow-lg">
          ...
          <p className={value >= 0 ? "text-green-600" : "text-red-600"}>
            {`Change: ${value >= 0 ? "+" : ""}${value.toLocaleString(
              undefined,
              {
          ...
          <p className="text-gray-600 text-sm">{`Bucket: ${
            data.index + 1
          }/100`}</p>
        </div>
      );
          ...
          >
            {changesData.map((day) => (
              <option key={day.date} value={day.date}>
                {day.date} (Total: {day.total >= 0 ? "+" : ""}
                {day.total.toFixed(2)} BTC)
              </option>
            ))}
          </select>
        </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                >
                  {selectedDayInfo.total >= 0 ? "+" : ""}
                  {selectedDayInfo.total.toFixed(2)} BTC
                </span>
              </div>
              <div>
                <span className="font-semibold text-gray-700">
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <ResponsiveContainer width="100%" height={500}>
          ...
          >
            ...
            <Bar dataKey="distributionChange" name="Distribution Change">
              {chartData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={entry.distributionChange >= 0 ? "#10b981" : "#ef4444"}
                />
              ))}
            </Bar>
          </BarChart>
        ...
        <div className="mt-4 text-sm text-gray-600">
          <p>
            <span className="font-semibold">Note:</span> These are day-over-day
            changes in UTXO distribution
          </p>
          <p>
            <span className="text-green-600 font-semibold">Green bars:</span>{" "}
            Increased BTC in this price range |{" "}
            <span className="text-red-600 font-semibold">Red bars:</span>{" "}
            Decreased BTC in this price range
          </p>
          <p>
            The total daily change ({selectedDayInfo?.total.toFixed(2)} BTC)
            represents the net change across all price buckets (mostly from new
            mining)
          </p>
        </div>
      </div>
